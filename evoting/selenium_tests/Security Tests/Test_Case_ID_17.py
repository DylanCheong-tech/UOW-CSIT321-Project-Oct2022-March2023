...
"""
Title: Access Control - Perform the Publish Vote Event Final Result operation without any login session information provided

Descriptions:
Test the system to be able check the authenticate before allowing the user to access the resources. 
The publish vote event final result is a POST action, where the attacker tries to script the POST request without any successful login attempts, the system should be able identify the unauthorized request.   
...
driver.execute_script(
	...
	form.action = "/evoting/eventowner/event/finalresult/publish/8";
	form.method = "POST";
...
query = """SELECT COUNT(*) FROM evoting_voteevent WHERE eventNo = %s and status = 'RP'"""

mycursor.execute(query, (8, ))

result = mycursor.fetchone()

assert int(result[0]) == 0

print("Security Test 17 Passed !")
